...
`open_in_ida` 是 proxy 侧的生命周期工具。它会使用 `IDA_PATH` 或 `config.conf` 中的 `ida_path` 解析 IDA 可执行文件，并通过子进程环境变量 `IDA_MCP_AUTO_START=1` 和预留好的 `IDA_MCP_PORT` 请求插件自动启动。默认会追加 `-A`；如果 `extra_args` 里已经显式传了 `-A`，则不会重复追加。
...
由于默认 `open_in_ida_use_autonomous=true`，IDA 会以 batch/autonomous 模式启动。它更适合无人值守自动化，也可能减少部分交互确认流程，但这不等价于正常手工逆向时的 GUI 启动：交互式对话框可能被抑制，依赖人工确认的 loader / 插件 / UI 流程可能表现不同。如果你的目标是日常手工分析，可将 `open_in_ida_use_autonomous=false`。
...
* `open_in_ida_use_autonomous` 默认值是 `true`，所以 `open_in_ida` 会默认追加 `-A`；如果 `extra_args` 里已经显式传了 `-A`，则不会重复追加
* `-A` 会让 IDA 进入 batch/autonomous 启动模式；如果希望默认保持常规交互式 GUI 启动行为，可将 `open_in_ida_use_autonomous=false`
